...
Promise.all(parts.map(async p=>{const r=await fetch(p,{cache:'no-store'});if(!r.ok)throw new Error(p+' HTTP '+r.status);return r.text()})).then(xs=>{
 ...
 js=js.replace("cards:CARDS.filter(x=>['red','green'].includes(x.color)).map(x=>x.id),","cards:CARDS.filter(x=>['red','green'].includes(x.color)&&!['colpo_in_testa','fabbro_ninjitsu'].includes(x.id)).map(x=>x.id),");
 (0,eval)(js);
