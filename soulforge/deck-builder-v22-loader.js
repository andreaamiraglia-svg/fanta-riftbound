...
Promise.all(parts.map(async p=>{const r=await fetch(p,{cache:'no-store'});if(!r.ok)throw new Error(p+' HTTP '+r.status);return r.text()})).then(xs=>{
 ...
 js=js.replace("['berserk','Berserk','red','Magia','Istantanea'],","['berserk','Berserk','red','Magia','Istantanea'],['spacca_teste','Spacca Teste','red','Magia','Risposta'],['colpo_in_testa','Colpo in Testa','red','Magia','Base'],");
 ...
 js=js.replace("cards:CARDS.filter(x=>['red','green'].includes(x.color)).map(x=>x.id),","cards:CARDS.filter(x=>['red','green'].includes(x.color)&&!['colpo_in_testa','fabbro_ninjitsu','spacca_teste'].includes(x.id)).map(x=>x.id),");
 (0,eval)(js);
