...
export function publicView(state:any,p:any){
 ...
 if(v?.board)v.board.monsters=(v.board.monsters||[]).map((m:any)=>{const raw=state?.board?.monsters?.find((x:any)=>String(x.uid)===String(m.uid));return {...m,pow:rawMonsterPow(state,raw||m)};});
 return v;
}
