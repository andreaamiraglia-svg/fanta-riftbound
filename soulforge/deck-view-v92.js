...
function deckStats(deck){
 ...
 const costs=[0,1,2,3,4,5].map(n=>({label:n===5?'5+':String(n),count:main.filter(x=>n===5?(Number(x.cost)||0)>=5:(Number(x.cost)||0)===n).length})),peak=Math.max(1,...costs.map(x=>x.count));
 const colors=[['red','Rosso'],['green','Verde'],['black','Nero'],['blue','Blu'],['orange','Arancione']].map(([id,label])=>({label,count:all.filter(x=>x.color===id).length,cls:'sf-stat-'+id}));
 ...
 return `<aside class="sf-deck-stats"><div class="sf-deck-stats-summary"><div class="sf-deck-stat"><strong>${all.length}</strong><span>Carte</span></div><div class="sf-deck-stat"><strong>${avg.toFixed(1)}</strong><span>Costo medio</span></div></div><div class="sf-deck-stat-box"><div class="sf-deck-stat-head"><span>⚡ Curva dei costi</span><small>MEDIA ${avg.toFixed(1)}</small></div><div class="sf-deck-curve">${costs.map(x=>`<div class="sf-deck-curve-col"><span class="sf-deck-curve-count">${x.count}</span><i class="sf-deck-curve-bar" style="height:${Math.max(2,x.count/peak*72)}px"></i><span class="sf-deck-curve-label">${x.label}</span></div>`).join('')}</div></div><div class="sf-deck-stat-box"><div class="sf-deck-stat-head"><span>Colori</span><small>${colors.filter(x=>x.count).length} ATTIVI</small></div>${meter(colors,all.length||1)}</div><div class="sf-deck-stat-box"><div class="sf-deck-stat-head"><span>Tipi di carta</span><small>${types.filter(x=>x.count).length} TIPI</small></div>${meter(types,all.length||1)}</div></aside>`;
}
