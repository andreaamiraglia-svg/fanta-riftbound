(()=>{
 function install(){
  ...
  const next=function(){
   ...
   main.prepend(strip);
   const controls=main.querySelector('.hand-title>.controls');
   if(controls){
    const pass=controls.querySelector('#passPriority, #passMain'),s=session.state;
    if(pass){
     pass.textContent=s.stack?.length?'Passa priorità':s.combat?'Fine combattimento':'Fine turno';
     pass.setAttribute('aria-label',pass.textContent);
    }
    controls.classList.add('sf-battle-controls');main.append(controls);
   }
