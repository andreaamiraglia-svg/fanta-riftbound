...
const direct=id=>ART[id]?BASE+ART[id]:'';
...
function resolved(id){
 const own=direct(id);if(own)return own;
 try{return typeof window.sfArtUrl21==='function'?String(window.sfArtUrl21(id)||''):''}catch{return''}
}
...
function ensureImg(el){
 ...
 const src=resolved(id);if(!src)return;
 let img=el.matches?.('img')?el:el.querySelector?.(':scope > img');
 if(!img){
  img=document.createElement('img');
  const cl=classFor(el);if(cl)img.className=cl;
...
function patch(root=document){
 if(root?.matches?.(SELECTOR))ensureImg(root);
 root?.querySelectorAll?.(SELECTOR).forEach(ensureImg);
}
function installResolver(){
 const cur=window.sfArtUrl21;if(cur?.__sfNewSetArt72)return;
 const prev=cur;
 const fn=id=>direct(String(id))||(typeof prev==='function'?prev(id):'');
...
document.addEventListener('mouseover',e=>{const el=e.target instanceof Element?e.target.closest(SELECTOR):null;if(el)ensureImg(el)},true);
window.addEventListener('sf-blue-ready',()=>setTimeout(()=>patch(),0));
setTimeout(()=>patch(),100);setTimeout(()=>patch(),500);setTimeout(()=>patch(),1500);setTimeout(()=>patch(),3000);
