...
function leaderboard(){
  const username=String(D?.user?.username||'').toLowerCase();
  const ownTeam=D?.myTeam ? D.leaderboard.find(t=>(D.myTeam.id&&t.id===D.myTeam.id)||(username&&String(t.owner||'').toLowerCase()===username)||(D.myTeam.name&&t.name===D.myTeam.name)) : null;
  const ordered=ownTeam?[ownTeam,...D.leaderboard.filter(t=>t!==ownTeam)]:D.leaderboard;
  const rankBanner=ownTeam?`<div class="myRankBanner"><div><span class="eyebrow">La tua posizione</span><strong>Sei attualmente <b>#${ownTeam.rank}</b></strong><small>su ${D.leaderboard.length} squadre</small></div><div class="myRankNumber">#${ownTeam.rank}</div></div>`:'';
  A.innerHTML=`<span class="eyebrow">Live standings</span><h1>Classifica.</h1><p class="lede leaderboardLead">Il giocatore con la corona è il leader della squadra: i suoi punti valgono ×2.</p>${rankBanner}<div class="table"><table><thead><tr><th>#</th><th>Squadra</th><th>Giocatori</th><th>Budget</th><th>Punti</th></tr></thead><tbody>${ordered.map(t=>{const mine=t===ownTeam;return`<tr class="${mine?'myTeamRow':''}"><td class="pts">${t.rank}</td><td><b>${e(t.name)}</b>${mine?'<span class="myTeamTag">LA TUA SQUADRA</span>':''}<div class="muted">${e(t.owner)}</div></td><td>${t.players.map(p=>`<span class="pill ${p.isLeader?'leaderPill':''}">${p.isLeader?'♛ ':''}${e(p.name)}${p.isLeader?' ×2':''}</span>`).join('')}</td><td>${t.cost}/${D.config.budget} G</td><td class="pts">${t.points}</td></tr>`}).join('')}</tbody></table></div>`;
}
