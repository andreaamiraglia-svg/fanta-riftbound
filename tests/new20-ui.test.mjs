...
assert.equal(black.name,'Richiamo del Branco (Nero)');assert.equal(green.name,'Richiamo del Branco (Verde)');assert.equal(black.color,'black');assert.equal(green.color,'green');assert.equal(black.cost,2);assert.equal(green.cost,0);assert.equal(pools.V27_ART[black.id],'richiamo-del-branco.webp');assert.equal(pools.V27_ART[green.id],'richiamo-del-branco-verde.webp');
